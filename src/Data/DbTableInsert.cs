...
namespace DevZest.Data
{
        ...
        where T : class, IModelReference, new()
    {
            ...
            where TSource : class, IModelReference, new()
        {
            ...
            var rowCount = await target.DbSession.InsertAsync(source, target, columnMapper, joinTo, identityMappings, ct);
            ...
            where TSource : class, IModelReference, new()
        {
            ...
            var identityColumn = dataSet.Model.Columns[target.Model.GetIdentity(false).Column.Ordinal];
        ...
        {
            using (var reader = await target.DbSession.ExecuteDbReaderAsync(identityOutput, ct))
            {
                int ordinal = 0;
        ...
        public static async Task<int> ExecuteAsync<TSource>(DbTable<T> target, DbQuery<TSource> source, IReadOnlyList<ColumnMapping> columnMappings, IReadOnlyList<ColumnMapping> join, CancellationToken ct)
            where TSource : class, IModelReference, new()
        {
            ...
            where TSource : class, IModelReference, new()
        {
            ...
            return target.UpdateOrigin(source, result.RowCount);
        }
