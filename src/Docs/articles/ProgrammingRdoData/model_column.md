...
You can add column annotations via Model Visualizer tool window as described in the previous topic. You're encouraged to develop your own custom column annotation, make sure it's decorated with [ModelDesignerSpec](xref:DevZest.Data.Annotations.Primitives.ModelDesignerSpecAttribute) so that your custom annotation will be friendly to Model Visualizer.
