...
{
    [TestClass]
    public class _ByteGeneartorTests
    {
        [TestMethod]
        public void _ByteGenerator_CS()
        {
            var dataSet = DataSet<_ByteModel>.Create();
            dataSet.AddRows(2);
            var _ = dataSet._;
            _.Column[0] = null;
            _.Column[1] = 1;
            ...
            using (var g = new DataSetGenerator(dataSet, LanguageNames.CSharp))
            {
                ...
                Assert.AreEqual("DevZest.Data.DbInit.TestModels._ByteModel", referencedTypes[2]);
                ...
                Assert.AreEqual("DataSet<_ByteModel> result = DataSet<_ByteModel>.Create().AddRows(2);", statements[1]);
                Assert.AreEqual("_ByteModel _ = result._;", statements[2]);
                Assert.AreEqual("_.SuspendIdentity();", statements[3]);
                Assert.AreEqual(@"_.Column[0] = null;", statements[4]);
                Assert.AreEqual(@"_.Column[1] = 1;", statements[5]);
        ...
        public void _ByteGenerator_VB()
        {
            ...
            using (var g = new DataSetGenerator(dataSet, LanguageNames.VisualBasic))
            {
                ...
                Assert.AreEqual("Dim result As DataSet(Of _ByteModel) = DataSet(Of _ByteModel).Create().AddRows(2)", statements[0]);
                Assert.AreEqual("Dim x As _ByteModel = result.Entity", statements[1]);
                Assert.AreEqual("x.SuspendIdentity()", statements[2]);
                Assert.AreEqual(@"x.Column(0) = Nothing", statements[3]);
                Assert.AreEqual(@"x.Column(1) = CByte(1)", statements[4]);
