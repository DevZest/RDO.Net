...
{
    [TestClass]
    public class _EnumGeneartorTests
    {
        [TestMethod]
        public void _EnumGenerator_CS()
        {
            var dataSet = DataSet<_EnumModel>.Create();
            dataSet.AddRows(2);
            var _ = dataSet._;
            _.Column[0] = null;
            _.Column[1] = SalesOrderStatus.Approved;
            ...
            using (var g = new DataSetGenerator(dataSet, LanguageNames.CSharp))
            {
                ...
                Assert.AreEqual("DevZest.Data.DbInit.TestModels._EnumModel", referencedTypes[2]);
                Assert.AreEqual("DevZest.Data.Primitives.ModelExtensions", referencedTypes[3]);
                Assert.AreEqual("DevZest.Samples.AdventureWorksLT.SalesOrderStatus", referencedTypes[4]);

                var statements = g.GetStatements().ToArray();
                Assert.AreEqual(9, statements.Length);
                Assert.AreEqual("{", statements[0]);
                Assert.AreEqual("DataSet<_EnumModel> result = DataSet<_EnumModel>.Create().AddRows(2);", statements[1]);
                Assert.AreEqual("_EnumModel _ = result._;", statements[2]);
                Assert.AreEqual("_.SuspendIdentity();", statements[3]);
                Assert.AreEqual(@"_.Column[0] = null;", statements[4]);
                Assert.AreEqual("_.Column[1] = SalesOrderStatus.Approved;", statements[5]);
        ...
        public void _EnumGenerator_VB()
        {
            ...
            using (var g = new DataSetGenerator(dataSet, LanguageNames.VisualBasic))
            {
                ...
                Assert.AreEqual("Dim result As DataSet(Of _EnumModel) = DataSet(Of _EnumModel).Create().AddRows(2)", statements[0]);
                Assert.AreEqual("Dim x As _EnumModel = result.Entity", statements[1]);
                Assert.AreEqual("x.SuspendIdentity()", statements[2]);
                Assert.AreEqual(@"x.Column(0) = Nothing", statements[3]);
                Assert.AreEqual("x.Column(1) = SalesOrderStatus.Approved", statements[4]);
