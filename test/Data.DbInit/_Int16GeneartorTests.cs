...
{
    [TestClass]
    public class _Int16GeneartorTests
    {
        [TestMethod]
        public void _Int16Generator_CS()
        {
            var dataSet = DataSet<_Int16Model>.Create();
            dataSet.AddRows(2);
            var _ = dataSet._;
            _.Column[0] = null;
            _.Column[1] = 100;
            ...
            using (var g = new DataSetGenerator(dataSet, LanguageNames.CSharp))
            {
                ...
                Assert.AreEqual("DevZest.Data.DbInit.TestModels._Int16Model", referencedTypes[2]);
                ...
                Assert.AreEqual("DataSet<_Int16Model> result = DataSet<_Int16Model>.Create().AddRows(2);", statements[1]);
                Assert.AreEqual("_Int16Model _ = result._;", statements[2]);
        ...
        [TestMethod]
        public void _Int16Generator_VB()
        {
            var dataSet = DataSet<_Int16Model>.Create();
            dataSet.AddRows(2);
            var _ = dataSet._;
            ...
            using (var g = new DataSetGenerator(dataSet, LanguageNames.VisualBasic))
            {
                ...
                Assert.AreEqual("Dim result As DataSet(Of _Int16Model) = DataSet(Of _Int16Model).Create().AddRows(2)", statements[0]);
                Assert.AreEqual("Dim x As _Int16Model = result.Entity", statements[1]);
                Assert.AreEqual("x.SuspendIdentity()", statements[2]);
                Assert.AreEqual(@"x.Column(0) = Nothing", statements[3]);
                Assert.AreEqual(@"x.Column(1) = 100S", statements[4]);
