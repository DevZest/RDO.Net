...
{
    [TestClass]
    public class _Int64GeneartorTests
    {
        [TestMethod]
        public void _Int64Generator_CS()
        {
            var dataSet = DataSet<_Int64Model>.Create();
            dataSet.AddRows(2);
            var _ = dataSet._;
            _.Column[0] = null;
            _.Column[1] = 100;
            ...
            using (var g = new DataSetGenerator(dataSet, LanguageNames.CSharp))
            {
                ...
                Assert.AreEqual("DevZest.Data.DbInit.TestModels._Int64Model", referencedTypes[2]);
                ...
                Assert.AreEqual("DataSet<_Int64Model> result = DataSet<_Int64Model>.Create().AddRows(2);", statements[1]);
                Assert.AreEqual("_Int64Model _ = result._;", statements[2]);
                Assert.AreEqual("_.SuspendIdentity();", statements[3]);
                Assert.AreEqual(@"_.Column[0] = null;", statements[4]);
                Assert.AreEqual(@"_.Column[1] = 100L;", statements[5]);
        ...
        [TestMethod]
        public void _Int64Generator_VB()
        {
            var dataSet = DataSet<_Int64Model>.Create();
            dataSet.AddRows(2);
            var _ = dataSet._;
            ...
            using (var g = new DataSetGenerator(dataSet, LanguageNames.VisualBasic))
            {
                ...
                Assert.AreEqual("Dim result As DataSet(Of _Int64Model) = DataSet(Of _Int64Model).Create().AddRows(2)", statements[0]);
                Assert.AreEqual("Dim x As _Int64Model = result.Entity", statements[1]);
                Assert.AreEqual("x.SuspendIdentity()", statements[2]);
                Assert.AreEqual(@"x.Column(0) = Nothing", statements[3]);
                Assert.AreEqual(@"x.Column(1) = 100L", statements[4]);
