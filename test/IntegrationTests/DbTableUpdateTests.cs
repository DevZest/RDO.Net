...
namespace DevZest.Data
{
    ...
    public class DbTableUpdateTests : AdventureWorksTestsBase
    {
        ...
        public void DbTable_Update_without_source()
        {
            ...
            {
                var count = db.ProductCategories.Where(x => x.ProductCategoryID > 2).CountAsync().Result;
                ...
                Assert.AreEqual(count, db.ProductCategories.Where(x => x.ModifiedDate == newModifiedDate).CountAsync().Result);
            }
        }
        ...
        public void DbTable_Update_from_Scalar()
        {
            ...
            using (var db = new ProductCategoryMockDb().InitializeAsync(OpenDbAsync(log).Result).Result)
            {
                var dataSet = db.ProductCategories.ToDataSetAsync().Result;
                Assert.IsTrue(dataSet.Count > 1);
                var newModifiedDate = new DateTime(2015, 11, 19);
                dataSet._.ModifiedDate[0] = newModifiedDate;

                db.ProductCategories.Update(dataSet, 0).ExecuteAsync().Wait();
                Assert.AreEqual(1, db.ProductCategories.Where(x => x.ModifiedDate == newModifiedDate).CountAsync().Result);
            }
        }
        ...
        public void DbTable_Update_from_DataSet()
        {
            ...
            using (var db = new ProductCategoryMockDb().InitializeAsync(OpenDbAsync(log).Result).Result)
            {
                ...
                db.ProductCategories.Update(dataSet).ExecuteAsync().Wait();
                Assert.AreEqual(count, db.ProductCategories.Where(x => x.ModifiedDate == newModifiedDate).CountAsync().Result);
            }
        }
